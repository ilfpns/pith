...
uint8_t find_next_task(uint8_t start_idx) {
    for (uint8_t base = 1; base <= task_count; base++) {
        ...
        else if (tcb_pool[idx].priority > current_task->priority && tcb_pool[idx].state == TASK_READY) {
            return idx;
        }
        else if (tcb_pool[idx].priority == current_task->priority) {
            return find_rr_task(idx);
        }
